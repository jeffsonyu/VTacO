...
from src.layers import ResnetBlockFC, Resnet18, Resnet34
...
from src.encoder.manolayer import ManoLayer
...
class LocalPoolPointnet(nn.Module):
    ...
    def __init__(self, c_dim=128, dim=3, hidden_dim=128, scatter_type='max', 
                 ...
                 out_mano=False, out_dim=None, manolayer_kwargs=None):
        ...
        if manolayer_kwargs is not None:
            self.mano_layer = ManoLayer(**manolayer_kwargs)
            
        if self.out_mano and self.plane_type == ['xz', 'xy', 'yz']:
            self.fc_mano = nn.Linear(c_dim*3, out_dim)
            
        if self.out_mano and self.plane_type == 'grid':
            self.fc_mano = nn.Linear(c_dim, out_dim)
    ...
    def forward(self, p):
        ...
        if self.out_mano:
            ...
            feature_mano = torch.cat([fea[key] for key, _ in fea.items()], 1) # B * (c_dim*3) * p_reso^3
            
            
            if 'grid' in fea.keys():
                feature_m = F.adaptive_avg_pool3d(feature_mano, (1, 1, 1)).squeeze(-1).squeeze(-1).squeeze(-1)
            else:
                feature_m = F.adaptive_avg_pool2d(feature_mano, (1, 1)).squeeze(-1).squeeze(-1)

            
            fea = {}
            fea_m = self.fc_mano(feature_m)
            fea['mano_param'] = fea_m
            
            if self.out_dim > 30:
            
                wrist_pos = torch.zeros(fea_m.size()[0], 3, requires_grad=True).to(fea_m.get_device())
                fea_m_full = torch.cat((wrist_pos, fea_m[:, 6:]), 1).to(fea_m.get_device())

                mano_info = self.mano_layer(fea_m_full)
                fea['mano_verts'] = mano_info[0]
                fea['mano_joints'] = mano_info[1]
                fea['mano_faces'] = self.mano_layer.th_faces
            
        return fea

    def forward_mano(self, fea_m_full):
