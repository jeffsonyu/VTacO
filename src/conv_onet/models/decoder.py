...
class LocalDecoder(nn.Module):
    ...
    def __init__(self, dim=3, c_dim=128,
                 hidden_size=256, n_blocks=5, leaky=False, sample_mode='bilinear', padding=0.1, with_contact=False):
        super().__init__()
        self.c_dim = c_dim
        ...
        self.fc_p_img = nn.Linear(dim+c_dim, hidden_size)
        ...
        if with_contact:
            self.fc_out_contact = nn.Linear(hidden_size, 1)
    ...
    def forward_contact(self, p, c_plane, **kwargs):
        ...
        out = self.fc_out(self.actvn(net))
        out_contact = self.fc_out_contact(self.actvn(net))
        out = out.squeeze(-1)
...
class AttentionDecoder(nn.Module):
    ...
    def __init__(self, dim=3, c_dim=128, input_size=2048,
                 hidden_size=256, n_blocks=5, leaky=False, sample_mode='bilinear', padding=0.1, with_contact=False):
        super().__init__()
        self.c_dim = c_dim
        self.n_blocks = n_blocks
