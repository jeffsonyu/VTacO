...
# Inferencer
def get_inferencer(model, optimizer, generator, cfg, device):
    ''' Returns a inferencer instance.
    ...
    '''
    method = cfg['method']
    inferencer = method_dict[method].config.get_inferencer(
        model, optimizer, generator, cfg, device)
    return inferencer
