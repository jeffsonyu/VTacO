...
class SequenceDataset(data.Dataset):
    ...
    def __init__(self, dataset_folder, fields, split=None,
        ...
        self.models_origin = self.models
        self.models = self.models_origin[2:-2]
        # precompute
    ...
    def get_one_item(self, idx):
        ...
        '''
        category = self.models_origin[idx]['category']
        model = self.models_origin[idx]['model']
        ...
        for field_name, field in self.fields.items():
            ...
            if isinstance(field_data, dict):
                for k, v in field_data.items():
                    ...
                    if k is None:
                        data[field_name] = v.astype(np.float32)
                    else:
                        if k == "name":
                            data['%s.%s' % (field_name, k)] = v
                        else:
                            data['%s.%s' % (field_name, k)] = v.astype(np.float32)
                    # print(v.dtype)
            else:
    ...
    def __getitem__(self, idx):
        data_back_2 = self.get_one_item(idx)
        data_back_1 = self.get_one_item(idx + 1)
        data_now = self.get_one_item(idx + 2)
        data_front_1 = self.get_one_item(idx + 3)
        data_front_2 = self.get_one_item(idx + 4)
        return [data_back_2, data_back_1, data_now, data_front_1, data_front_2]
