...
class PointsField(Field):
    ...
    def load(self, model_path, idx, category):
        ...
        points_obj = points_dict['points_obj'].astype(np.float32)
        np.random.shuffle(points_obj)
        points_obj = points_obj[:2048]
        contact = points_dict['contact'].astype(np.float32)
        
        pc_hand = points_dict['pc_hand'].astype(np.float32)
        mano_param = points_dict['mano'].astype(np.float32)
        wrist_rot = points_dict['wrist_rot'].astype(np.float32)
        cam_pos = points_dict['cam_pos'].astype(np.float32)
        cam_rot = points_dict['cam_rot'].astype(np.float32)
        cam_rot = cam_rot/180 * np.pi 
...
class PointCloudField(Field):
    ...
    def load(self, model_path, idx, category):
        ...
        points = pointcloud_dict['points'].astype(np.float32)
        normals = pointcloud_dict['normals'].astype(np.float32)
        pc_ply = pointcloud_dict['pc_ply'].astype(np.float32)
        images = pointcloud_dict['img']
        depths = pointcloud_dict['depth'].astype(np.float32)
        touch_success = pointcloud_dict['touch_success']
        
        noise = np.random.normal(0, 7, images.shape)
        images = np.clip(images + noise, 0, 255)/255
        images = images/255
